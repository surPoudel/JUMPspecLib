...
initial_FDR = float(config["specLib"]["initial_FDR"])
...
if filter_method == "0":
    ...
    printDF2Rank1 = mergeSearchResults(search_result_file, filter_col, initial_FDR)
    reqd_columns = ['L_ID','Peptide_ID','charge','exp','scan','prec_MZ','Prec_MZ_theoretical','[M+H]+','RT','JDscore','abs_dPrecMZ','pepLength','Type']

    write_log("Input PSMs for filtering = {}\n".format(printDF2Rank1.shape[0]))
    filteredDF_rapid = FDR_Target_Decoy(printDF2Rank1,filter_col)


    filteredScans = publicationTablesRapidFilter(filteredDF_rapid, jump_f_outFolder, float(FDR),reqd_columns, L_ID_uniqProtDict, uniquePeptideTrack)
    ...
    # target, decoy = RT_inference_MS2based(exp_mzxml, printDF2Rank1)
    ...
    printDF2Rank_1 = dRT_inference_mergeSearchResults(search_result_file, filter_col, initial_FDR)
    printDF2Rank1 = printDF2Rank_1.sort_values(by=["JDscore"], ascending=False)
    ...
    filteredScans = finalPublicationTables(psms_firstSearch, jump_f_outFolder, reqd_columns, L_ID_uniqProtDict, uniquePeptideTrack)
