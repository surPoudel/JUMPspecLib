...
def get_rt_jdscore(psms, mzxml): #psms = df of idtxt
...
def get_rt(psms, mzxml): #psms = df of idtxt
    ...
    mzXMLBaseName = os.path.basename(mzxml).split(".mzXML")[0]
    psms_run = psms.loc[psms["Outfile"].str.contains(mzXMLBaseName+"\.")]    # Extract PSMs from FTLD_Batch2_F50.mzXML
...
#     print (runs)
    for run in runs:
        ...
        runName = os.path.basename(run).split(".mzXML")[0]
